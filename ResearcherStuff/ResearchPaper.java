...
public class ResearchPaper {
    ...
    public String getCitation(String format) {
        if (format.equalsIgnoreCase("PlainText")) {
            return title + " published in " + journal + " on " + datePublished + ", DOI: " + doi;
        } else if (format.equalsIgnoreCase("Bibtex")) {
            return "@article{" + doi + ", title={" + title + "}, journal={" + journal + "}, year={" + datePublished.getYear() + "}}";
        } else {
    ...
    public void notifySubscribers() {
        for (User subscriber : subscribers) {
            System.out.println("Notifying " + subscriber.getuserName() + " about " + title);
        }
    }
